...
<style>
:root { --blue:#a71f25; --green:#17705d; --red:#c73632; --gold:#b88938; --ink:#19312d; --gray:#687671; --bg:#e9eeea; --orange:#b96a27; }
* { margin:0; padding:0; box-sizing:border-box; }
body { font-family:-apple-system,system-ui,"PingFang SC","SF Pro","Helvetica Neue",sans-serif; background:var(--bg); color:var(--ink); }
.hero { background-color:#173b35; background-image:linear-gradient(rgba(255,255,255,.045) 1px,transparent 1px),linear-gradient(90deg,rgba(255,255,255,.045) 1px,transparent 1px); background-size:20px 20px; color:#fff; padding:19px 16px 17px; border-bottom:5px solid #a71f25; }
.hero-inner { max-width:600px; margin:auto; display:flex; align-items:center; justify-content:space-between; gap:12px; }
.brand { font-family:"Songti SC","STSong",serif; font-size:25px; font-weight:900; letter-spacing:.12em; }
.tagline { opacity:.82; font-size:12px; margin-top:3px; }
.admin-link { color:#fff; opacity:.78; text-decoration:none; font-size:12px; padding:7px 9px; border:1px solid rgba(255,255,255,.32); border-radius:9px; }
.notice { background:#f8f5e9; border-left:4px solid var(--gold); color:#4e5146; padding:11px 13px; margin-bottom:12px; font-size:13px; line-height:1.5; }
#map { height:50vh; width:100%; min-height:250px; }
.panel { padding:16px; max-width:600px; margin:0 auto; }
.card { background:#fdfefb; border:1px solid #cfd9d3; border-radius:7px; padding:16px; margin-bottom:12px; box-shadow:3px 3px 0 rgba(25,49,45,.07); }
...
.btn-primary { background:var(--blue); color:#fff; box-shadow:inset 0 0 0 1px rgba(255,255,255,.18); }
...
.chips { display:flex; gap:7px; overflow-x:auto; padding:1px 0 5px; scrollbar-width:none; }
.chip { border:1px solid #e4d4c0; background:#fffaf3; color:#5d4433; border-radius:999px; padding:8px 12px; white-space:nowrap; font-size:12px; cursor:pointer; }
.suggestions { border:1px solid #e5d8c9; border-radius:10px; margin-top:7px; overflow:hidden; display:none; }
.suggestion { padding:10px 12px; border-bottom:1px solid #eee4d8; font-size:12px; cursor:pointer; background:#fff; }
.suggestion:last-child { border:0; }
.history-item { display:flex; justify-content:space-between; gap:10px; padding:9px 0; border-bottom:1px solid #f0e7dc; font-size:12px; }
...
<body>
<header class="hero"><div class="hero-inner"><div><div class="tagline" style="letter-spacing:.18em;margin-bottom:4px">LAO WANG · CHECK-IN</div><div class="brand">${escapeHtml(config.brandName || "老王打卡")}</div><div class="tagline">iPhone 网络定位选点工具</div></div><a class="admin-link" href="/admin">管理后台</a></div></header>
...
<div class="panel">
  <div class="notice">${escapeHtml(config.announcement || "请选择目标地点并储存到设备。")}</div>
  ...
  <div class="card">
    <h3>快速选点</h3>
    <div class="chips" id="commonPlaces"></div>
  </div>
  <div class="card">
    <h3>选择目标位置</h3>
...
<script>
const SITE_CONFIG = ${publicConfig};
...
/* ---- Common places & history ---- */
function renderCommonPlaces() {
  const el = document.getElementById('commonPlaces');
  const places = SITE_CONFIG.commonPlaces || [];
  el.innerHTML = places.length ? places.map((p, i) => '<button class="chip" onclick="loadCommon('+i+')">'+escHtml(p.name)+'</button>').join('') : '<span class="status">管理员暂未配置</span>';
}
...
function addHistory(name) {
  ...
  list.unshift({ name:name || '地图选点', lat, lon, time:Date.now() });
...
document.getElementById('searchInput').addEventListener('input', () => { clearTimeout(suggestTimer); suggestTimer=setTimeout(suggest,350); });
